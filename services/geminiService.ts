...
export const isAIAvailable = (): boolean => {
  ...
  console.log('🔑 API Key Check:', {
    ...
    startsWithAIza: manualKey?.startsWith('AIza'),
    keyPreview: manualKey ? `${manualKey.substring(0, 10)}...` : 'none'
  });
  return !!(manualKey && manualKey.startsWith('AIza') && manualKey.length >= 30);
};
...
export const analyzeCurriculum = async (curriculumText: string): Promise<string[]> => {
    ...
    if (!isAIAvailable()) {
        console.log("Curriculum analysis skipped - API not available");
