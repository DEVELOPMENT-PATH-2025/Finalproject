        ...
        if (data.section === 'body' && data.column.index === 1) {
          ...
          if (rowIndex >= 0 && rowIndex % 3 === 0) {
            ...
            if (q && q.imageUrl) {
              ...
              const y = data.cell.y + data.cell.height - imgHeight - 3; // Bottom of question cell
              try {
                doc.addImage(q.imageUrl, 'PNG', x, y, imgWidth, imgHeight);
              } catch (e) {
                console.error("Error adding image to PDF:", e);
      ...
      didDrawCell: (data) => {
        if (data.section === 'body' && data.column.index === 0) {
          ...
          if (q && q.imageUrl) {
            ...
            const imgHeight = 25;
            const x = data.cell.x + (data.cell.width - imgWidth) / 2;
            const y = data.cell.y + data.cell.height - imgHeight - 3;
            try {
              doc.addImage(q.imageUrl, 'PNG', x, y, imgWidth, imgHeight);
            } catch (e) {
              console.error("Error adding image to PDF:", e);
        ...
        if (data.section === 'body' && data.column.index === 1) {
          ...
          if (q) {
            ...
            if (q.imageUrl) {
              ...
              const imgHeight = 30;
              const x = data.cell.x + (data.cell.width - imgWidth) / 2;
              // Position image after question 'a' text, before 'OR'
              const y = data.cell.y + (data.cell.height * 0.1) + 10; 
              try {
                doc.addImage(q.imageUrl, 'PNG', x, y, imgWidth, imgHeight);
              } catch (e) {
                console.error("Error adding image to PDF:", e);
